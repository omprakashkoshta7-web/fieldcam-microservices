...
const renderInvoicePdf = (invoice, res) => {
  ...
  doc.fontSize(10)
    .fillColor(light)
    .font('Helvetica')
    .text(invoice.invoiceNumber || 'INV-DRAFT', 50, 85);

  const vendorName = invoice.vendorName || invoice.vendor?.profile?.name || 'Vendor';
  const vendorEmail = invoice.vendorEmail || invoice.vendor?.email || '';
  doc.fontSize(11)
    .fillColor('#1A1A1A')
    .font('Helvetica-Bold')
    .text(vendorName, 350, 50, { align: 'right', width: 200 });
  doc.fontSize(9)
    .fillColor(gray)
    .font('Helvetica')
    .text('Licensed Contractor', 350, 66, { align: 'right', width: 200 })
    .text(vendorEmail, 350, 78, { align: 'right', width: 200 });
  ...
  if (invoice.project) {
    const ref = `${invoice.project.projectNumber || ''} - ${
      invoice.project.address || invoice.project.title || ''
    }`;
  ...
  doc.text('DESCRIPTION', 60, y + 7)
    .text('QTY', 330, y + 7)
    .text('RATE', 380, y + 7)
    .text('AMOUNT', 460, y + 7, { align: 'right', width: 75 });
  y += 22;

  const items = invoice.lineItems?.length
    ? invoice.lineItems
    : [{ desc: invoice.description || 'Service', qty: 1, rate: invoice.amount, amount: invoice.amount }];
  items.forEach((item, i) => {
    doc.rect(50, y, 495, 24).fillColor(i % 2 === 0 ? '#FFFFFF' : '#FAFAFA').fill();
    doc.fontSize(10)
      .fillColor('#1A1A1A')
      .font('Helvetica')
      .text(item.desc || '', 60, y + 7, { width: 260 });
    doc.text(String(item.qty ?? 1), 330, y + 7, { width: 40 })
      .text(`${item.rate ?? 0}.00`, 380, y + 7, { width: 70 });
    doc.font('Helvetica-Bold').text(`${item.amount ?? 0}.00`, 460, y + 7, {
      align: 'right',
      width: 75,
    });
    y += 24;
  ...
  doc.text('Subtotal', 380, y).text(`${subtotal}.00`, 460, y, { align: 'right', width: 75 });
  y += 18;
  doc.text('Tax (8%)', 380, y).text(`${tax}.00`, 460, y, { align: 'right', width: 75 });
  y += 18;
  doc.moveTo(380, y).lineTo(545, y).strokeColor('#E5E7EB').lineWidth(0.5).stroke();
  y += 8;
  doc.fontSize(13)
    .fillColor('#1A1A1A')
    .font('Helvetica-Bold')
    .text('Total Due', 380, y)
    .text(`${total}.00`, 460, y, { align: 'right', width: 75 });
  y += 30;

  doc.moveTo(50, y).lineTo(545, y).strokeColor('#E5E7EB').lineWidth(0.5).stroke();
  y += 14;
  doc.fontSize(8).fillColor(light).font('Helvetica-Bold').text('VENDOR SIGNATURE', 50, y);
  y += 12;
  doc.fontSize(18).fillColor('#1A1A1A').font('Helvetica-Oblique').text(vendorName, 50, y);
  doc.fontSize(9).fillColor('#16A34A').font('Helvetica-Bold').text('Digitally Signed', 350, y + 6);

  doc.fontSize(8)
    .fillColor(light)
    .font('Helvetica')
    .text('Generated by FieldWork Cam', 50, 780, { align: 'center', width: 495 });
  doc.end();
};

exports.downloadPdf = async (req, res) => {
  try {
    const invoice = await Invoice.findOne({ _id: req.params.id, vendor: req.user._id }).populate(
      'project',
      'projectNumber address title client',
    );
...
exports.downloadAdminPdf = async (req, res) => {
  try {
    const invoice = await Invoice.findById(req.params.id)
      .populate('project', 'projectNumber address title client')
      .populate('vendor', 'profile.name email');
    if (!invoice) return res.status(404).json({ message: 'Invoice not found' });
    renderInvoicePdf(invoice, res);
  } catch (err) {
